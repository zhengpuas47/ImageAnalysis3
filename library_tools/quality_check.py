...
def _check_primer_usage(pb_records, fwd_primer, rev_primer, primer_len=20, _verbose=True):
    ...
    fwd_len = len(fwd_primer.seq[-primer_len:])
    rev_len = len(rev_primer.seq[-primer_len:].reverse_complement())
    
    for record in pb_records:
        if record.seq[:fwd_len] != fwd_primer.seq[-primer_len:]:
            if _verbose:
...
def _parsing_probe_sequence(record, add_rand_gap=0, primer_len=20, readout_len=20, target_len=42):
    ...
    # take in a seq record, parse primers
    _main_seq = record.seq
    _fwd_primer = _main_seq[:primer_len]
    _rev_primer = _main_seq[-primer_len:].reverse_complement()
    # parse the sequence and return a list of all included readouts (20mer,RC)
    readout_list = []
    ...
    _target = _main_seq[:target_len]

    return _target, readout_list, _fwd_primer, _rev_primer

def _parse_probe_name(record):
    """parse a probe name to acquire information"""
    _info_dict = {}
    _name = record.id
    # remove readouts
    _readout_name_list = _name.split('[')[1].split(']')[0].split(",")
    _name = _name.split('_readouts_[')[0]
    # remove pb_index
    if '_strand_' in _name:
        _info_dict['strand'] = _name.split('_strand_')[1]
        _name = _name.split('_strand_')[0]
    # remove pb_index
    if '_pos_' in _name:
        _info_dict['pb_index'] = int(_name.split('_pos_')[1])
        _name = _name.split('_pos_')[0]
    # remove _pb_id
    _pb_id = int(_name.split('_pb_')[1])
    _info_dict['reg_name'] = _name.split('_pb_')[0]
    
    return _info_dict, _readout_name_list, _pb_id

def _finding_readout_name(readout_list, readout_dict, readout_len=20, 
                          probe_readout_num=3, rc_site=False):
    ...
    for _site in readout_list:
        # rev_com readout sites if specified (should already been rc during parsing)
        if rc_site:
            _match_site = _site.reverse_complement()
        else:
            _match_site = _site
...
def _check_readout_to_region(reg_to_readout, pb_records, readout_dict, 
                             ...
                             primer_len=20, readout_len=20, add_rand_gap=0,
                             probe_readout_num=3,
                             verbose=True):
    ...
    for record in pb_records:
        ...
        _readout_list = _parsing_probe_sequence(record, target_len=target_len, primer_len=primer_len, 
                                                readout_len=readout_len, add_rand_gap=add_rand_gap)[1]
